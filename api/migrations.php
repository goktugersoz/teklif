...
function ensure_schema(): void
{
    ...
    $pdo = db();
    foreach (schema_statements(database_driver()) as $sql) {
        $pdo->exec($sql);
    }
    $done = true;
}
...
function schema_statements(string $driver): array
{
    if ($driver === 'pgsql') {
        return [
            ...
            "CREATE INDEX IF NOT EXISTS idx_request_items_request ON request_items(request_id, sort_order)",
            ...
            "CREATE INDEX IF NOT EXISTS idx_offers_request ON offers(request_id, submitted_at)",
